...
const Signup = () => {
    ...
    const onSave = async(data) =>{
        const status = true
        let title = 'Admin Authentication', type, text
        ...
        try {
            ...
            if(signupSuccess || !res.data.error){
                ...
                const response = await addNewStaff({ name, gender: data?.gender, contact, user: user.id })

                if(response?.error){
                    type = 'Error'
                    console.log(response)
                    text = response?.error?.data?.message || 'Admin user authentication error'
                }else{
                    type = 'Success'
                ...
                type = 'Error'
                text = res?.data?.message || 'An error occured while authenticating admin user!'
            }
        ...
        <div className='bg-gray-200 min-h-screen flex justify-center items-center'>
            ...
            <div className='container mx-auto flex justify-center items-center'>
                <div className='w-1.5/3 bg-white p-8 rounded-lg shadow-lg text-gray-800 my-6'>
                    <h2 className='text-xl font-bold text-center mb-8 border-b border-gray-200 pb-2'>Admin Authenticaton</h2>
