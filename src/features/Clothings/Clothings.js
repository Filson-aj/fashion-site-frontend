...
const Clothings = () => {
    ...
    if(isSuccess){
        ...
        let filteredIds = ids
        if(params){
            filteredIds = ids.filter(id => clothings.entities[id]?.clothingid?.toLowerCase().includes(params) || clothings.entities[id]?.name?.toLowerCase().includes(params) || clothings.entities[id]?.price?.toString().toLowerCase().includes(params) || clothings.entities[id]?.style?.toString().toLowerCase().includes(params) || clothings.entities[id]?.category?.toString().toLowerCase().includes(params))
        }
            ...
            content = (
                <>
                    <div className='grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 pt-4 px-4'>
                        {currentIds?.length > 0 && currentIds.map(clothingId => <Clothing key={clothingId} clothingId={clothingId} />)}
                    </div>
