...
const Customers = () => {
    ...
    const { 
        ...
        isError,
        error
     } = useGetCustomersQuery('customers', {
    ...
    if(isSuccess){
        ...
        let filteredIds = ids
        if(params){
            filteredIds = ids.filter(id => customers.entities[id]?.customerid?.toLowerCase().includes(params) || customers.entities[id]?.name?.firstname.toLowerCase().includes(params) || customers.entities[id]?.name?.surname.toLowerCase().includes(params) || customers.entities[id]?.name?.othername.toLowerCase().includes(params) || customers.entities[id]?.gender.toLowerCase().includes(params) || customers.entities[id]?.contact?.phone.toLowerCase().includes(params))
        }
            ...
            content = (
                <>
                    <div className='grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 pt-4 px-4'>
                        {currentIds?.length > 0 && currentIds.map(customerId => <Customer key={customerId} customerId={customerId} />)}
                    </div>
    ...
    <div className='container mx-auto bg-white text-gray-800 rounded-lg border shadow-lg w-full'>
        <div className='border-b border-gray-200 pb-2 flex items-center justify-between'>
            ...
            {status === 'Admin' && <Link className='bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-2 w-8 h-8
                shadow-lg rounded-full text-center mx-4 my-4 text-center' to={`/dashboard/customers/new`} title='Book a customer'>
