...
const NewCustomerForm = () => {
    useTitle('Create: New Customer')
    const {
        register,
        handleSubmit,
        formState: { errors }
    } = useForm({ resolver: yupResolver(customerSchema), mode: 'onBlur'})
    ...
    const onSave = async(data) =>{
        ...
        const status = true
        let title = 'New Customer Record', type, text
        ...
        try {
            ...
            if(signupSuccess || !res.data?.error){
                ...
                if(response?.error){
                    ...
                    type = 'Success'
                    text = response?.data?.message || 'Customer records has been saved successfully!'
                }
            }else{
                type = 'Error'
                text = res?.data?.message || 'An error occured while save new customer record!'
            }
        ...
        <div className='w-full px-6 py-6 bg-white dark:text-gray-800
            ...
            <form onSubmit={handleSubmit(onSave)}>
                <div className='flex justify-between items-center border-b border-gray-200 mb-4'>
                    <h2 className='text-xl font-bold text-center pb-2 '>New Customer</h2>
